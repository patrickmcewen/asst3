...
__global__ void kernelBoundCircles(int* circles_per_block) {
    ...
    for (int x = 0; x < cuConstRendererParams.gridDim_x; x++) {
        for (int y = 0; y < cuConstRendererParams.gridDim_y; y++) {
            ...
            float boxT = 1-((float)y * cuConstRendererParams.blockDim_y) / (float)cuConstRendererParams.imageHeight;
            float boxB = 1-((float)(y+1) * (cuConstRendererParams.blockDim_y)) / (float)cuConstRendererParams.imageHeight;
...
CudaRenderer::render() {
    ...
    float4* data = (float4*)(&image_data_print[4 * (205 * params.imageWidth + 389)]);
    printf("y = 205 (row), x = 389 (column): %f, %f, %f\n", data->x, data->y, data->z);
    cudaDeviceSynchronize();
}
